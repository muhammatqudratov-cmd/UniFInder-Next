...
const AddUniversity = ({ initialValues, ...props }: any) => {
	...
	const [insertUniversityData, setInsertUniversityData] = useState<UniversityInput>(initialValues);
	...
	const {
		loading: getUniversityLoading,
		data: getUniversityData,
		error: getUniversityError,
		refetch: getUniversityRefetch,
	} = useQuery(GET_UNIVERSITY, {
		fetchPolicy: 'network-only',
		variables: { input: router.query.universityId },
	});

	/** LIFECYCLE **/
	useEffect(() => {
		setInsertUniversityData({
			...insertUniversityData,
			universityName: getUniversityData?.getUniversity ? getUniversityData?.getUniversity?.universityName : '',
			universityTuition: getUniversityData?.getUniversity ? getUniversityData?.getUniversity?.universityTuition : 0,
			universityType: getUniversityData?.getUniversity ? getUniversityData?.getUniversity?.universityType : '',
			universityLocation: getUniversityData?.getUniversity ? getUniversityData?.getUniversity?.universityLocation : '',
			universityAddress: getUniversityData?.getUniversity ? getUniversityData?.getUniversity?.universityAddress : '',
			universityScholarship: getUniversityData?.getUniversity
				? getUniversityData?.getUniversity?.universityScholarship
				: false,
			universityDormitory: getUniversityData?.getUniversity
				? getUniversityData?.getUniversity?.universityDormitory
				: false,
			universityFaculties: getUniversityData?.getUniversity ? getUniversityData?.getUniversity?.universityFaculties : 0,
			universityCapacity: getUniversityData?.getUniversity ? getUniversityData?.getUniversity?.universityCapacity : 0,
			universityCampusSize: getUniversityData?.getUniversity
				? getUniversityData?.getUniversity?.universityCampusSize
				: 0,
			universityDesc: getUniversityData?.getUniversity ? getUniversityData?.getUniversity?.universityDesc : '',
			universityImages: getUniversityData?.getUniversity ? getUniversityData?.getUniversity?.universityImages : [],
		});
	}, [getUniversityLoading, getUniversityData]);
	...
	async function uploadImages() {
		try {
			...
			setInsertUniversityData({ ...insertUniversityData, universityImages: responseImages });
		} catch (err: any) {
	...
	const doDisabledCheck = () => {
		if (
			insertUniversityData.universityName === '' ||
			insertUniversityData.universityTuition === 0 || // @ts-ignore
			insertUniversityData.universityType === '' || // @ts-ignore
			insertUniversityData.universityLocation === '' || // @ts-ignore
			insertUniversityData.universityAddress === '' || // @ts-ignore
			insertUniversityData.universityScholarship === '' || // @ts-ignore
			insertUniversityData.universityDormitory === '' ||
			insertUniversityData.universityFaculties === 0 ||
			insertUniversityData.universityCapacity === 0 ||
			insertUniversityData.universityCampusSize === 0 ||
			insertUniversityData.universityDesc === '' ||
			insertUniversityData.universityImages.length === 0
		) {
			return true;
		}
	};

	const insertUniversityHandler = useCallback(async () => {
		try {
			const result = await createUniversity({
				variables: {
					input: insertUniversityData,
				},
			});

			await sweetMixinSuccessAlert('University created successfully');
			await router.push({
				pathname: '/mypage',
				query: 'myUniversities',
			});
		} catch (error) {
	...
	const updateUniversityHandler = useCallback(async () => {
		try {
			// @ts-ignore
			insertUniversityData._id = getUniversityData?.getUniversity?._id;
			...
			await sweetMixinSuccessAlert('University updated successfully');
			await router.push({
				pathname: '/mypage',
				query: 'myUniversities',
			});
	...
	console.log('+insertUniversityData', insertUniversityData);

	if (device === 'mobile') {
		return <div>ADD NEW UNIVERSITY MOBILE PAGE</div>;
	} else {
		return (
			<div id="add-university-page">
				<Stack className="main-title-box">
					<Typography className="main-title">Add New University</Typography>
					...
					<Stack className="config">
						<Stack className="description-box">
							<Stack className="config-column">
								...
								<input
									...
									placeholder={'Title'}
									value={insertUniversityData.universityName}
									onChange={({ target: { value } }) =>
										setInsertUniversityData({ ...insertUniversityData, universityName: value })
									}
								/>
							</Stack>
							...
							<Stack className="config-row">
								<Stack className="price-year-after-price">
									<Typography className="title">Tuition</Typography>
									<input
										type="text"
										className="description-input"
										placeholder={'Price'}
								...
								<Stack className="price-year-after-price">
									...
									<select
										...
										defaultValue={insertUniversityData.universityType || 'select'}
										value={insertUniversityData.universityType || 'select'}
										onChange={({ target: { value } }) =>
											// @ts-ignore
											setInsertUniversityData({ ...insertUniversityData, universityType: value })
										}
									>
										<>
							...
							<Stack className="config-row">
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										defaultValue={insertUniversityData.universityLocation || 'select'}
										value={insertUniversityData.universityLocation || 'select'}
										onChange={({ target: { value } }) =>
											// @ts-ignore
											setInsertUniversityData({ ...insertUniversityData, universityLocation: value })
										}
									>
										<>
											<option selected={true} disabled={true} value={'select'}>
											...
											{universityLocation.map((location: any) => (
								...
								<Stack className="price-year-after-price">
									...
									<input
										...
										className="description-input"
										placeholder={'Address'}
										value={insertUniversityData.universityAddress}
										onChange={({ target: { value } }) =>
											setInsertUniversityData({ ...insertUniversityData, universityAddress: value })
										}
									/>
								</Stack>
							</Stack>
							...
							<Stack className="config-row">
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertUniversityData.universityScholarship ? 'yes' : 'no'}
										defaultValue={insertUniversityData.universityScholarship ? 'yes' : 'no'}
										onChange={({ target: { value } }) =>
											setInsertUniversityData({ ...insertUniversityData, universityScholarship: value === 'yes' })
										}
									>
								...
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertUniversityData.universityDormitory ? 'yes' : 'no'}
										defaultValue={insertUniversityData.universityDormitory ? 'yes' : 'no'}
										onChange={({ target: { value } }) =>
											setInsertUniversityData({ ...insertUniversityData, universityDormitory: value === 'yes' })
										}
									>
										<option disabled={true} selected={true}>
											Select
							...
							<Stack className="config-row">
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertUniversityData.universityFaculties || 'select'}
										defaultValue={insertUniversityData.universityFaculties || 'select'}
										onChange={({ target: { value } }) =>
											setInsertUniversityData({ ...insertUniversityData, universityFaculties: parseInt(value) })
										}
									>
								...
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertUniversityData.universityCapacity || 'select'}
										defaultValue={insertUniversityData.universityCapacity || 'select'}
										onChange={({ target: { value } }) =>
											setInsertUniversityData({ ...insertUniversityData, universityCapacity: parseInt(value) })
										}
									>
										<option disabled={true} selected={true} value={'select'}>
											Select
								...
								<Stack className="price-year-after-price">
									...
									<select
										className={'select-description'}
										value={insertUniversityData.universityCampusSize || 'select'}
										defaultValue={insertUniversityData.universityCampusSize || 'select'}
										onChange={({ target: { value } }) =>
											setInsertUniversityData({ ...insertUniversityData, universityCampusSize: parseInt(value) })
										}
									>
										<option disabled={true} selected={true} value={'select'}>
											Select
							...
							<Typography className="university-title">University Description</Typography>
							<Stack className="config-column">
								...
								<textarea
									...
									value={insertUniversityData.universityDesc}
									onChange={({ target: { value } }) =>
										setInsertUniversityData({ ...insertUniversityData, universityDesc: value })
									}
								></textarea>
							</Stack>
						</Stack>

						<Typography className="upload-title">Upload photos of your university</Typography>
						<Stack className="images-box">
							...
							<Stack className="gallery-box">
								{insertUniversityData?.universityImages.map((image: string) => {
						...
						<Stack className="buttons-row">
							{router.query.universityId ? (
								<Button className="next-button" disabled={doDisabledCheck()} onClick={updateUniversityHandler}>
									<Typography className="next-button-text">Save</Typography>
								</Button>
							) : (
								<Button className="next-button" disabled={doDisabledCheck()} onClick={insertUniversityHandler}>
									<Typography className="next-button-text">Save</Typography>
								</Button>
							)}
						</Stack>
...
AddUniversity.defaultProps = {
	initialValues: {
		...
		universityDormitory: false,
		universityFaculties: 0,
		universityCapacity: 0,
		universityCampusSize: 0,
		universityDesc: '',
		universityImages: [],
	},
};
