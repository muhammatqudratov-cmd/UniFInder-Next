...
const RecentlyVisited: NextPage = () => {
	...
	const [recentlyVisited, setRecentlyVisited] = useState<University[]>([]);
			...
			<div id="my-favorites-page">
				...
				<Stack className="favorites-list-box">
					{recentlyVisited?.length ? (
						recentlyVisited?.map((university: University) => {
							return <UniversityCard university={university} recentlyVisited={true} />;
						})
					) : (
						<div className={'no-data'}>
							<img src="/img/icons/icoAlert.svg" alt="" />
							<p>No Recently Visited Universities found!</p>
						</div>
					)}
				</Stack>
				{recentlyVisited?.length ? (
					<Stack className="pagination-config">
						...
						<Stack className="total-result">
							<Typography>
								Total {total} recently visited university {total > 1 ? 'ies' : 'y'}
							</Typography>
						</Stack>
					</Stack>
				) : null}
