...
interface TrendUniversityCardProps {
	university: University;
	likeUniversityHandler: any;
}

const TrendUniversityCard = (props: TrendUniversityCardProps) => {
	...
	if (device === 'mobile') {
		return (
			<Stack className="trend-card-box" key={university._id}>
				<Box
					component={'div'}
					className={'card-img'}
				...
				<Box component={'div'} className={'info'}>
					...
					<p className={'desc'}>{university.universityDesc ?? 'no description'}</p>
				...
				<Box component={'div'} className={'info'}>
					...
					<div className={'bott'}>
						...
						<div className="view-like-box">
							...
							<Typography className="view-cnt">{university?.universityViews}</Typography>
							<IconButton onClick={() => likeUniversityHandler(user, university?._id)} color={'default'}>
								{university?.meLiked && university?.meLiked[0]?.myFavorite ? (
									<FavoriteIcon style={{ color: 'red' }} />
								) : (
									<FavoriteIcon />
...
export default TrendUniversityCard;
