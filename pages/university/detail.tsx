...
import Review from '../../libs/components/university/Review';
...
import { GET_COMMENTS, GET_UNIVERSITIES, GET_UNIVERSITY } from '../../apollo/user/query';
...
const UniversityDetail: NextPage = ({ initialComment, ...props }: any) => {
	...
	const [destinationUniversities, setDestinationUniversities] = useState<University[]>([]);
	const [commentInquiry, setCommentInquiry] = useState<CommentsInquiry>(initialComment);
	const [universityComments, setUniversityComments] = useState<Comment[]>([]);
	const [commentTotal, setCommentTotal] = useState<number>(0);
	const [insertCommentData, setInsertCommentData] = useState<CommentInput>({
		commentGroup: CommentGroup.UNIVERSITY,
		commentContent: '',
		commentRefId: '',
	});
	...
	const {
		...
		variables: { input: universityId },
		skip: !universityId,
		notifyOnNetworkStatusChange: true,
		onCompleted(data: T) {
			if (data?.getUniversity) setUniversity(data?.getUniversity);
			if (data?.getUniversity) setSlideImage(data?.getUniversity?.universityImages[0]);
		},
	});
	...
	const {
		...
		variables: {
			input: {
				...
				search: {
					locationList: university?.universityLocation ? [university?.universityLocation] : [],
				},
			},
		},
		skip: !universityId && !university,
		notifyOnNetworkStatusChange: true,
		onCompleted(data: T) {
			if (data?.getUniversities.list) setDestinationUniversities(data?.getUniversities?.list);
		},
	});
	...
	const {
		...
		notifyOnNetworkStatusChange: true,
		onCompleted(data: T) {
			if (data?.getComments.list) setUniversityComments(data?.getComments?.list);
	...
	useEffect(() => {
		if (router.query.id) {
			setUniversityId(router.query.id as string);
			setCommentInquiry({
				...commentInquiry,
				search: {
		...
		try {
			...
			await getUniversitiesRefetch({
				input: {
					...
					direction: Direction.DESC,
					search: {
						locationList: [university?.universityLocation],
					},
				},
			});
	...
	if (device === 'mobile') {
		return <div>UNIVERSITY DETAIL PAGE</div>;
	} else {
		return (
			<div id={'university-detail-page'}>
				<div className={'container'}>
					<Stack className={'university-detail-config'}>
						<Stack className={'university-info-config'}>
							<Stack className={'info'}>
								<Stack className={'left-box'}>
									<Typography className={'title-main'}>{university?.universityName}</Typography>
									<Stack className={'top-box'}>
										<Typography className={'city'}>{university?.universityLocation}</Typography>
										...
										<Typography className={'date'}>{moment().diff(university?.createdAt, 'days')} days ago</Typography>
									</Stack>
								...
								<Stack className={'right-box'}>
									<Stack className="buttons">
										<Stack className="button-box">
											...
											<Typography>{university?.universityViews}</Typography>
										</Stack>
										<Stack className="button-box">
											{university?.meLiked && university?.meLiked[0]?.myFavorite ? (
												...
												<FavoriteBorderIcon
													fontSize={'medium'}
													// @ts-ignore
													onClick={() => likeUniversityHandler(user, university?._id)}
												/>
											)}
											<Typography>{university?.universityLikes}</Typography>
										</Stack>
									</Stack>
						...
						<Stack className={'university-desc-config'}>
							<Stack className={'left-config'}>
								<Stack className={'options-config'}>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
											<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none">
												<path
													d="M8.5 11c1.93 0 3.5-1.79 3.5-4s-1.57-4-3.5-4S5 4.79 5 7s1.57 4 3.5 4Zm7.5-1c1.66 0 3-1.57 3-3.5S17.66 3 16 3s-3 1.57-3 3.5S14.34 10 16 10Zm-7.5 2c-2.67 0-8 1.34-8 4v2.5C1 19.33 1.67 20 2.5 20h12c.83 0 1.5-.67 1.5-1.5V16c0-2.66-5.33-4-8-4Zm9.04.08c.97.86 1.96 2.13 1.96 3.92v2.5c0 .27-.05.52-.13.75H21.5c.83 0 1.5-.67 1.5-1.5v-1.75c0-2.18-3.5-3.44-5.96-3.92Z"
													fill="#1C1B18"
												/>
											</svg>
										</Stack>
									...
									<Stack className={'option'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{university?.universityType}</Typography>
										</Stack>
									</Stack>
								</Stack>
								<Stack className={'prop-desc-config'}>
									<Stack className={'top'}>
										<Typography className={'title'}>University Description</Typography>
										<Typography className={'desc'}>{university?.universityDesc ?? 'No Description!'}</Typography>
									</Stack>
									<Stack className={'bottom'}>
										<Typography className={'title'}>University Details</Typography>
										<Stack className={'info-box'}>
											<Stack className={'left'}>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Price</Typography>
													<Typography className={'data'}>${formatterStr(university?.universityTuition)}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>University Size</Typography>
											...
											<Stack className={'right'}>
												...
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>University Type</Typography>
													<Typography className={'data'}>{university?.universityType}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
								...
								<Stack className={'address-config'}>
									...
									<Stack className={'map-box'}>
										...
										></iframe>
										<Stack className={'map-tooltip'}>
											<Typography className={'street'}>Geumhogangbyeon-ro</Typography>
											<Typography className={'city'}>Dong-gu, Daegu</Typography>
										</Stack>
										<Box component={'div'} className={'map-pin'}>
								...
								{commentTotal !== 0 && (
									<Stack className={'reviews-config'}>
										...
										<Stack className={'review-list'}>
											{universityComments?.map((comment: Comment) => {
								...
								<Stack className={'info-box'}>
									...
									<Stack className={'image-info'}>
										<img
											...
											src={
												university?.memberData?.memberImage
										...
										<Stack className={'name-phone-listings'}>
											<Link href={`/member?memberId=${university?.memberData?._id}`}>
												<Typography className={'name'}>{university?.memberData?.memberNick}</Typography>
											</Link>
											<Stack className={'phone-number'}>
												...
												<Typography className={'number'}>{university?.memberData?.memberPhone}</Typography>
											</Stack>
						...
						{destinationUniversities.length !== 0 && (
							<Stack className={'similar-universities-config'}>
								<Stack className={'title-pagination-box'}>
									<Stack className={'title-box'}>
										<Typography className={'main-title'}>Destination University</Typography>
								...
								<Stack className={'cards-box'}>
									...
									>
										{destinationUniversities.map((university: University) => {
											return (
												<SwiperSlide className={'similar-homes-slide'} key={university.universityName}>
													<UniversityBigCard
...
export default withLayoutFull(UniversityDetail);
